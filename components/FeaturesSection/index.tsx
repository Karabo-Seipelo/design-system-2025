...
const List = ({ features }: { features: Feature[] }) => {
    return (<ul className="flex flex-col gap-10 lg:flex-1">
        {features.map((feature, index) => (<li className="flex flex-row gap-4 md:gap-5" key={feature.title + " " + index}>
...
const Grid = ({ features }: { features: Feature[] }) => (<div className="grid grid-cols-1 gap-2 md:grid-cols-2 lg:grid-cols-3">
    {features.map((feature, index) => (<div className="flex flex-col items-center gap-2" key={feature.title + " " + index}>
        <div className="h-fit rounded-full bg-white p-3 shadow-[0_1px_3px_rgba(0,0,0,0.1)]">
            <Image src={feature.icon} alt={feature.title} width={0} height={0} className="min-w-[24px] md:w-full min-h-[24px] lg:h-[24px]" />
        </div>
            ...
            <div className="flex h-full flex-col imtes-start px-3 py-12 md:px-4 md:py-16 lg:items-center lg:justify-center lg:px-24 lg:py-24">
                <section className="flex flex-col gap-12 md:gap-16">
                    ...
                    <main className="flex flex-col gap-12 lg:flex-row lg:items-start lg:gap-8">
                        {features && (feautureLayout === "grid" ? <Grid features={features} /> : <List features={features} />)}
                        {imageUrl && <div className={`flex w-full lg:flex-1 ${orientation === "left" ? 'order-first' : 'order-last'}`}>
