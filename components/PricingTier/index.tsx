...
const Price = ({ tag, price, description, features, buttons, classes, plan, billedAt, highlight = false} : Price) => {
    ...
    return (
        <div className={`flex flex-col gap-y-8 w-full bg-white border-solid rounded-lg ${classes} ${highlight ? "border-2 border-solid border-indigo-700 shadow-[0_25px_50px_rgba(0,0,0,0.25)]" : "border border-solid border-neutral-200"}`}>
           {highlight && tag && <div className="bg-indigo-50 py-4 rounded-tl-lg rounded-tr-lg items-center self-stretch font-bold text-xl text-center text-indigo-700">{tag}</div>}
           <div className={`flex flex-col gap-8 h-full ${highlight ? "px-8 pb-8" : "p-8"}`}>
                <div className="flex flex-col text-left gap-4">
                    ...
                    <h2 className={`font-semibold text-5xl ${highlight ? "text-indigo-700" : "text-neutral-900"}`}>{price}</h2>
                ...
                <TabGroup className="flex flex-col gap-5 lg:gap-10">
                    <TabList className="flex flex-row items-center gap-4 self-stretch outline-none w-full md:w-[280px] md:m-auto">
                        {tiers && tiers.map((tier, index) => {
                            return <Tab key={`${tier.subscription}-${index}`} className="justify-center items-center gap-1.5 grow px-4 py-2.5 rounded border-solid border-neutral-200 bg-white data-[selected]:border-[0.5px]">{tier.subscription}</Tab>
                        })}
                    </TabList>
                    <TabPanels className="flex flex-row gap-5 md:w-full">
                        {tiers && tiers.map((tier, index) => {
                            return <TabPanel key={`${tier.subscription}-${index++}`} className="flex flex-col w-full lg:flex-row gap-y-8 lg:gap-x-8 mt-12">
