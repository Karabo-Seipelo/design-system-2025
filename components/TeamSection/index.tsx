...
const TeamSection = ({title, subTitle, description, team}: TeamSectionProps) => {
    return (
        <div className="w-full rounded bg-white shadow-sm">
            <div className="flex h-full gap-10 lg:gap-20 flex-col px-3 py-12 md:px-4 md:py-16 lg:p-24">
                ...
                <main className="flex flex-col gap-12 lg:flex-row lg:items-start lg:gap-8">
                    <ul className="flex flex-col gap-12 self-stretch w-full md:grid md:grid-cols-2 md:gap-x-8 md:gap-y-16 lg:grid-cols-4" role="list">
                    {team?.map(({name, role, imageUrl, description}, index) => (
                        <li key={name + " " + index} className="flex flex-col gap-6 self-stretch md:w-[336px] lg:w-[280px]" role="listitem">
                            <Image src={imageUrl} alt={`${name}'s picture`} width={0} height={0} className="w-full h-auto md:h-[296px] object-cover" />
